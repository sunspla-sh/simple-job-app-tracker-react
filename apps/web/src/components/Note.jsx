...
export const Note = ({ id, createdAt, updatedAt, content, jobAppId, createMode, jobApp, setJobApp }) => {
  ...
  const handleSubmitCreateNote = async e => {
    ...
    try {
      ...
      setCreateNoteSuccessMessage('Note created successfully!')
      setTimeout(() => setCreateNoteSuccessMessage(''), 5000);
      toggleIsCreating();
    } catch (err) {
      console.log('err while creating note', err);
      setCreateNoteErrorMessage(err.message);
      setTimeout(() => setCreateNoteErrorMessage(''), 5000);
    }
  ...
  const [editNoteSuccessMessage, setEditNoteSuccessMessage] = useState('');

  const [editNoteErrorMessage, setEditNoteErrorMessage] = useState('');
  ...
  const handleSubmitEditNote = async e => {
    ...
    try {
      ...
      toggleIsEditing();
      setEditNoteSuccessMessage('Note edited successfully!')
      setTimeout(() => setEditNoteSuccessMessage(''), 5000);
    } catch (err) {
      console.log('err while editing note', err);
      setEditNoteErrorMessage(err.message);
      setTimeout(() => setEditNoteErrorMessage(''), 5000);
    }
  }
    ...
    return (
      <>
        ...
        {createNoteErrorMessage && (
          <p className='note_create-error-message'>Error: {createNoteErrorMessage}</p>
        )}
        { createNoteSuccessMessage && (
          <p className='note_create-success-message'>{createNoteSuccessMessage}</p>
        )}
      </>
  ...
  if (isEditing) {
    ...
    return (
      <>
        ...
        {editNoteErrorMessage && (
          <p className='note_edit-error-message'>Error: {editNoteErrorMessage}</p>
        )}
      </>
    );
  ...
  return (
    <>
      ...
      { editNoteSuccessMessage && (
        <p className='note_edit-success-message'>{editNoteSuccessMessage}</p>
      )}
    </>
  );
